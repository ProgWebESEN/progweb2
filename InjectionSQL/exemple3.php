...
/**
 * ===================================================
 * Solution 1 : Utilisation des requêtes préparées
 * ===================================================
 */
// Inclure le fichier
...
if(isset($_GET["connexion"])){
    ...
    $sql = "SELECT * FROM utilisateur WHERE login = :login AND mot_passe = :pass";
    $resultat = $db->prepare($sql);
    $resultat->bindParam(":login", $login, PDO::PARAM_STR);
    $resultat->bindParam(":pass", $mot_passe, PDO::PARAM_STR);
    echo "<p>$sql</p>";
    try{
        $resultat->execute();
...
<head>
    <meta charset="UTF-8">
    <title>Injection SQL non autorisée</title>
    <style>
        input[type="text"]{
            width: 50%;
        }
